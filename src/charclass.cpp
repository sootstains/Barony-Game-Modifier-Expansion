...
void initClass(const int player)
{
	...
	else if ( client_classes[player] == CLASS_ARCANIST )
	{
		...
		{
			// ammo
			item = newItem(QUIVER_FIRE, SERVICABLE, 0, 25, 0, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[1].item = item2->uid;
			free(item);
	...
	else if ( client_classes[player] == CLASS_HUNTER )
	{
		...
		if ( isLocalPlayer )
		{
			...
			item = newItem(QUIVER_SILVER, SERVICABLE, 0, 25, 0, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[2].item = item2->uid;
			free(item);

			item = newItem(QUIVER_SILVER, SERVICABLE, 0, 15, 0, true, nullptr); // split stacks to obey stack size limit
			item2 = itemPickup(player, item);
			free(item);

			item = newItem(QUIVER_LIGHTWEIGHT, SERVICABLE, 0, 25, 0, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[3].item = item2->uid;
			free(item);

			item = newItem(QUIVER_LIGHTWEIGHT, SERVICABLE, 0, 15, 0, true, nullptr); // split stacks to obey stack size limit
			item2 = itemPickup(player, item);
			free(item);
