				...
				else
				{
					...
					switch (my->skill[1])
					{
						...
						case 3:
						{
							...
							Item* equipment[] = {stats[i]->helmet, stats[i]->breastplate, stats[i]->gloves, stats[i]->shoes, 
												stats[i]->shield, stats[i]->weapon, stats[i]->cloak, stats[i]->mask};

							for (int slot = 0; slot < 8; ++slot)
							{
								if (equipment[slot] && equipment[slot]->status < EXCELLENT && equipment[slot]->type != TOOL_TORCH)
								{
									equipment[slot]->status = static_cast<Status>(equipment[slot]->status + 1);
								}
							}
							...
							if ( multiplayer == SERVER && i > 0 && !players[i]->isLocalPlayer() ) // tell client to repair gear
							{
								strcpy((char*)net_packet->data, "REP8");
								net_packet->address.host = net_clients[i - 1].host;
								net_packet->address.port = net_clients[i - 1].port;
								net_packet->len = 4;
								sendPacketSafe(net_sock, -1, net_packet, i - 1);
